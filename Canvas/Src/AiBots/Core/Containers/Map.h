...
namespace Rio
{
	...
	namespace MapFn
	{
		...
		void clear(Map<TKey, TValue>& m)
		{
			...
			typename Map<TKey, TValue>::Node node(*m.data.allocator);
			node.pair.first = TKey();
			node.pair.second = TValue();
			node.left = MapInternal::NIL;
			node.right = MapInternal::NIL;
			node.parent = MapInternal::NIL;
			node.color = MapInternal::BLACK;
			VectorFn::pushBack(m.data, node);
		}
