...
namespace Rio
{
	...
	private:
		...
		uint8_t* bufferBegin;
		uint8_t* bufferEnd;
		uint8_t* bufferCurrentPtr;
		size_t allocChunkSize;
		uint8_t buffer[SIZE];
	};

	// To reduce unneeded template instantiation pre-made sizes could be used
	// For blobs more than 4K use scratch allocator or heap allocator
