...
void renderDemo(struct NVGcontext* vg, float width, float height, struct DemoData* data)
{
	float x;
	float y; 
	float popx;
...
int _main_(int /*_argc*/, char** /*_argv*/)
{
	...
	while (!entry::processEvents(app.width, app.height, app.debug, app.reset, &app.mouseState))
	{
		app.update();
	}
